...
spans.forEach(el => {
    el.addEventListener('click', function () {
        if (generatedLottoNumbers.length === 6) return alert(`Już wylosowałeś 6 liczb! Wciśnij przycisk 'Nowy los', aby puścić kolejny zakład!`)
        if (chosenNumbers.length < 6) {
            if (el.className === 'lotto__number--circle active') return alert(`Wybrałeś już liczbę ${el.getAttribute('data-number')}. Wybierz inną!`)
